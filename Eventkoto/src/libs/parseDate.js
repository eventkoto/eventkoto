...
export function parseDateString(date){
    let d = new Date(date)
    return (`${months[d.getMonth()]} ${d.getDate()}, ${d.getFullYear()} - ${wkDay[d.getDay()]} ${String(d.getHours() == 0 ? 12 : d.getHours() < 13 ? d.getHours() : d.getHours()-12).padStart(2, "0")} : ${String(d.getMinutes()).padStart(2, "0")} ${d.getHours() > 12 ? "PM" : "AM"}`)
}